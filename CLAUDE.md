...
A research experiment testing whether Transformer matrices can learn with **no persistent floating-point master weights**. Code and pressure nibbles are packed into one `uint8` per weight; FP32 row scales and optional row RMS EMA are explicit support state. The current implementation includes fused tiled backward/update and experimental CUDA matmul backends. This is one-byte state packing, not entropy-packed 2.32/2.81-bit storage. Read `docs/superpowers/specs/2026-06-20-ratchet-training-design.md` for the precision boundary and update rule.
...
- Ratchet matrices persist **one packed `uint8` code/pressure tensor**, one FP32 scale per output row, and an optional FP32 row RMS EMA. Never add an FP32/BF16 `Parameter` that mirrors a code matrix. `audit_no_master_weights` (run via `lat audit`) enforces this at runtime — keep it passing.
...
- **`ratchet.py`** — the core. `RatchetLinear` holds packed integer state; the fused backend tiles the effective-weight gradient and applies the pressure/code update during autograd backward. `bucket_pressure` converts RMS-normalized gradients into integer pressure (descent direction). Boundary outward-moves are recorded as **blocked moves** so pressure can't wind up. `audit_no_master_weights` walks a model and reports violations + state bytes.
...
- **`checkpoint.py`** — version-2 safetensors + validated JSON metadata. Training saves CPU and active-CUDA RNG, selected seed, and per-module leak counters; tokenizer identity is validated before mutation and RNG is restored immediately before continuation. Legacy generation remains supported. `data.py` builds the char corpus with a deterministic final-10% validation split.

The three control arms (`lat controls`): **FP32** swaps every ratchet matrix for a trained `nn.Linear`; **frozen-quinary/septenary** train embeddings + RMSNorm while packed state, scales, optional EMA, leak counters, and ratchet statistics remain unchanged.
